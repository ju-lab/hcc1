...
for i in fastq/W*/*1.fastq.gz; do
	outdir=bam_hg38/`echo $i | sed -r 's!.*/(.*)/.*!\1!'`
	sample=`basename $i | sed 's/1.fastq.gz//'|sed 's/_$//;s/_R$//'`
	echo $outdir $sample $i ${i/1.fastq.gz/2.fastq.gz}
	run_bwa2.sh $sample $i ${i/1.fastq.gz/2.fastq.gz} -o $outdir -l log/hg38_bwa --process bwa,picard --script
	exit 0
done
exit 0
